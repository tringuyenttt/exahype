...
if [ ! -d ${asagi_build} ]; then
    ...
    mkdir -p ${asagi_build}
    cd ${asagi_build}
    cmake_flags="-DCMAKE_BUILD_TYPE=Debug -DNOMPI=OFF -DNONUMA=OFF ${cmake_set_compiler} -DNUMA_LIBRARY=${numa_dir}/lib -DNUMA_INCLUDE_DIR=${numa_dir}/include"
